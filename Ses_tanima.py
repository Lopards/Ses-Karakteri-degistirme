...
def classify(filename):
    signal, rate = librosa.load(filename, sr=None, mono=False)# yöntemini kullanarak ses dosyasını yükler ve ses sinyalini ve örnekleme hızını signal ve rate değişkenlerine atar. 

    # Ses sinyalini analiz etmek ve sonucu etiketlemek için gereken işlemler burada yapılacak.

    if np.mean(signal[0]) > np.mean(signal[1]): #Belirtilen eksen boyunca verilen verilerin (dizi öğeleri) aritmetik ortalamasını alır 
        gender = "Kadın"
    else:
        gender = "Erkek"
